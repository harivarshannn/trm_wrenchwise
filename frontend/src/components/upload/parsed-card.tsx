  ...
  const formatEducation = (list: EducationItem[]) =>
    (list || [])
      .map((item) => {
        ...
        return [item.degree?.trim(), item.level?.trim()].filter(Boolean).join(" — ");
      })
      .filter(Boolean)
      .join("\n");
  ...
  const formatExperience = (list: ExperienceItem[]) =>
    (list || [])
      .map((item) => {
        if (!item) return "";
        if (typeof item === "string") return (item as string).trim();
        return [item.company?.trim(), item.role?.trim(), item.years?.trim()].filter(Boolean).join(" — ");
      })
      .filter(Boolean)
      .join("\n");
  ...
  const renderExperienceList = (list: ExperienceItem[]) => {
    ...
    const formattedList = (list || [])
      .map((item) => {
        ...
        let years = item.years?.trim();
        if (years) {
          const lowerYears = years.toLowerCase();
          if (!lowerYears.includes("year") && !lowerYears.includes("yr")) {
            years = `${years} years`;
          }
        }
        return [company, role, years].filter(Boolean).join(" — ");
      })
      .filter(Boolean);
