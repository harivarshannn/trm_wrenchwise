...
class Settings:
    ...
    brevo_sender_email: str = "no-reply@trms.com"
    brevo_sender_name: str = "TRMS Recruitment"
...
def get_settings() -> Settings:
    ...
    if not jwt_secret:
        # In development, we can fallback, but in production we should fail.
        # However, to maintain current behavior while still allowing for strictness,
        # we will check if we are in a 'PROD' like environment.
        # For now, let's just make it required if we want to be safe.
    ...
    return Settings(
        ...
        brevo_sender_email=os.getenv("BREVO_SENDER_EMAIL", "no-reply@trms.com").strip(),
        brevo_sender_name=os.getenv("BREVO_SENDER_NAME", "TRMS Recruitment").strip(),
    )
